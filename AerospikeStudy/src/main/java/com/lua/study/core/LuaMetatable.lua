...
--以下为返回对象元表：
mtb = getmetatable(mytable)   -- 这回返回mymetatable
print(type(mtb))
print(type(mytable))
...
Lua 查找一个表元素时的规则，其实就是如下 3 个步骤:
    ...
    3.判断元表有没有 __index 方法，如果 __index 方法为 nil，则返回 nil；如果 __index 方法是一个表，
    则重复 1、2、3；如果 __index 方法是一个函数，则返回该函数的返回值。
...
在对新索引键（newkey）赋值时（mytable.newkey = "新值2"），
